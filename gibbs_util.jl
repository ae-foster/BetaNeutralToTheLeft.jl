...
mutable struct GibbsSamplerControls
  ...
  n_iter::Int64 # = 50000 # 50000  # total number of Gibbs iterations to run
  n_burn::Int64 # = 10000   # burn-in
  n_thin::Int64 # = 100     # collect every `n_thin` samples
  n_print::Int64 # = 1000 # print status updates with timings every `n_print` iterations

  # set which components to update
  gibbs_psi::Bool # = true            # NTL Ψ paramters
  gibbs_alpha::Bool  # = true          # NTL alpha parameter
  gibbs_T::Bool #  = true  # arrival times
  gibbs_ia_params::Bool # = true     # arrival time distribution parameters
  gibbs_sigma::Bool # = true   # order of blocks in partition/vertices in graph
  ...
  alpha_fixed::Float64 # = 0.5 # when alpha is fixed (not to be updated)
  arrival_params_fixed::Vector{Float64} # when arrival distn parameters are fixed

end

# GibbsSamplerControls() = GibbsSamplerControls(50000,10000,100,1000,true,true,true,true,true,)
...
function GibbsSampler(G::GibbsSamplerControls,
  PP::Vector{Int},T_data::Vector{Int})
