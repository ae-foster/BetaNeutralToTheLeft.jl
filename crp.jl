...
# function crp_logpdf(theta::Float64,alpha::Float64,n::Int,k::Int,x::Int)
#   logp = log(theta + alpha*k) + lgamma(theta + n) - lgamma(theta + n + x) + lgamma(n + x - 1 - alpha*k) - lgamma(n - alpha*k)
#   return logp
# end
#
# function crp_logpdf(theta::Float64,alpha::Float64,n::Int,k::Int,v::Vector{Int})
#   logp = ( log(theta + alpha*k) + lgamma(theta + n) - lgamma(n - alpha*k) ) .- lgamma.((theta + n) .+ v) .+ lgamma.( v .+ (n - 1 - alpha*k))
#
# end
